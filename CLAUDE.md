...
Both live in `results/phase2/dod/` and are regenerated by one command each (see the reporting-code
...
- **`fig1_accumulation_within_scene.png`** -- the headline slice (m=5, depth 5), both deflation
  ...
  ```
  python scripts/plot_phase2_depth.py results/phase2/dod/phase2_librimix_5spk_scratch345.csv \
      --x-axis n_accepted_before --depth 5 \
      --out results/phase2/dod/fig1_accumulation_within_scene.png
  ```
...
- **`fig2_accumulation_across_scenes.png`** -- paired SI-SDR vs `no_recursion` at fixed depth 2,
  ...
  ```
  python scripts/plot_phase2_depth.py results/phase2/dod/phase2_librimix_{3,4,5}spk_scratch345.csv \
      --x-axis m --depth 2 \
      --out results/phase2/dod/fig2_accumulation_across_scenes.png
  ```
...
(replaced with exact paired differencing). Both figures now come from a committed command:
`plot_phase2_depth.py` gained safe defaults (error bars on, `--min-n 25`, paired-vs-control by
default on the `m` axis, terminal step auto-dashed) plus a computed control-flatness check, the
triplicated clipping rule was consolidated into `dagger/metrics/phase2_scores.py`, and
`tests/phase2/test_reporting_guards.py` now encodes all three shipped reporting defects as
regression fixtures (292 tests green). Previously: 2026-08-04 — PHASE 2 CLOSE-OUT (see the close-out block at the end of §5 Phase 2).
